          ...
          <div className="mt-10 grid gap-4 lg:grid-cols-3">
            {dealTiers.map((d) => (
              <Link
                key={d.slug}
                href={`/deals/${d.slug}`}
                className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${d.accent} p-6 text-white shadow-pop transition duration-300 hover:-translate-y-1 hover:shadow-gold`}
              >
                <p className="text-xs font-semibold uppercase tracking-wider text-white/80">{d.name}</p>
